...
/*   Updated: 2025/12/19 18:56:54 by bmartin-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int ft_add_line(t_info_map *info, char *line)
{
	...
	// Aquí ya no necesitas validar el rectángulo, porque ft_readd_file ya lo hizo justo antes

	// 1. Solo validamos muros laterales (usando el ancho que ya conocemos)
...
int	ft_check_final(t_info_map *info_map)
{
	...
	if (info_map->player != 1 || info_map->exit != 1
		...
		return (ft_print_error("Missing components (P, E or C)\n"));
	if (!ft_is_only_one(info_map->map[info_map->num_rows - 1]))
		return (ft_print_error("Map not closed by walls\n"));
	ft_free_area(info_map->map, info_map->num_rows);
	info_map->map = NULL;
	return (1);
}*/
